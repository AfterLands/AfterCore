 ...
 *
 * <p><b>Threading:</b> nenhum método deve bloquear a main thread.</p>
 */
public interface AfterCoreAPI {
    ...
    @NotNull ActionService actions();

    @NotNull CommandService commands();

    @NotNull ProtocolService protocol();
     ...
     *
     * <p>A action será executada considerando o scope definido na spec:</p>
     * <ul>
     ...
     * </ul>
     *
     * <p>Thread safety: Garante execução na main thread quando necessário.</p>
     *
     * @param spec Action spec parseada (via {@link ActionService#parse(String)})
     * @param viewer Player que está "vendo" (usado para scope VIEWER e referência de posição)
     */
     ...
     *
     * <p>Útil quando a origem da action não é a posição do viewer
     * (ex: explosão em uma localização específica).</p>
     *
